...
class Booking extends CI_Controller
{
	...
	public function index()
	{
		...
		if ($dtb < 1) {
			$this->session->set_flashdata('pesan', '<div class="alert alert-danger alert-dismissible fade show" role="alert"> Tidak ada buku di keranjang </div>');
			redirect(base_url());
		} else {
	...
	{
		// Mendapatkan URI dari URL saat ini
		$currentUrl = $_SERVER['REQUEST_URI'];

// Mendapatkan path dari URI
		$path = parse_url($currentUrl, PHP_URL_PATH);

// Menggunakan basename() untuk mendapatkan segmen terakhir
		$lastSegment = basename($path);

// Menampilkan segmen terakhir
		echo "Segmen Terakhir: " . $lastSegment;

		$id_buku = $lastSegment;

		// memilih data buku yang akan dimasukkan ke tabel 'temp' / keranjang melalui variabel $isi
		$d = $this->db->query("select * from buku where id ='$id_buku'")->row();

		// berupa data-data yang akan deisimpan kedalam tabel 'temp' / keranjang
		$isi = [
			'id_buku' => $id_buku,
		...
		// cek apakag buku yabg di klik booking sudah ada di keranjang
		$temp = $this->ModelBooking->getDataWhere('temp', ['id_buku' => $id_buku])->num_rows();

		$userid = $this->session->userdata('id_user');

		// cek jika sudah memasukan 3 buku untuk dibooking kedalam keranjang
	...
	public function bookingSelesai($where)
	{
		...
		$this->db->query(
			"UPDATE buku, temp SET buku.dibooking=buku.dibooking+1, buku.stok=buku.stok-1 WHERE buku.id=temp.id_buku"
		);
	...
	public function info()
	{
		...
		$data['temp'] = $this->db->query("select * from booking bo, booking_detail d, buku bu where d.id_booking=bo.id_booking and d.id_buku=bu.id and bo.id_user='$where'")->result_array();
		$this->load->view('templates/templates-user/header', $data);
